...
#define TEST_RECIPIENT_ADDRESS "0x729221d84D36acF19594D793D8Ca76C19eFc7b12"

BUINT8 g_binFormatKey[32];
BoatKeypairPriKeyCtx_config g_keypair_config;
BoatEthNetworkConfig g_ethereum_network_config;

BoatKeypairPriKeyCtx_config get_ethereum_keypair_settings()
{
    ...
    else
    {
        ...
        g_keypair_config.prikey_content.field_ptr = (BUINT8 *)g_ethereum_private_key_buf;
        g_keypair_config.prikey_content.field_len = strlen(g_ethereum_private_key_buf) + 1;
    }
...
BOAT_RESULT check_ethereum_keypairCtx(BoatKeypairPriKeyCtx *keypair2Ctx, BUINT8 keypair1Index, BCHAR *keypair1Name, BoatKeypairPriKeyCtx_config *keypair1Config)
{
...
START_TEST(test_001Keypair_0002CreateOneTimeKeypairSuccessWithName)
{
    ...
    ret = check_ethereum_keypairCtx(&keypair2_ctx, keypair1_index, "oneTime_keypair", &keypair1_config);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
...
START_TEST(test_001Keypair_0003CreateOneTimeKeypairSuccessTwice)
{
    ...
    /* 2. Create keypair2 */
    BoatKeypairPriKeyCtx_config keypair2_config = get_ethereum_keypair_settings();
    ...
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ret = check_ethereum_keypairCtx(&keypair_ctx, keypair2_index, "oneTime_keypair", &keypair2_config);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ...
    ret = BoATKeypair_GetKeypairList(&keypair_list);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ret = check_ethereum_keypair_in_keypairList(&keypair_ctx, &keypair_list);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    /* free memory of keypair_name*/
...
START_TEST(test_001Keypair_0010CreatePersistKeypairSuccess)
{
    ...
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ret = check_ethereum_keypairCtx(&keypair2_ctx, keypair1_index, "Persistkeypair1", &keypair1_config);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
...
START_TEST(test_001Keypair_0011Create2PersistKeypairSuccess)
{
    ...
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ret = check_ethereum_keypairCtx(&keypair3_ctx, keypair1_index, "Persistkeypair1", &keypair1_config);
    ck_assert_int_eq(ret, BOAT_SUCCESS);

    keypair4_ctx.keypair_name = NULL;
    ret = BoATKeypair_GetKeypairByIndex(&keypair4_ctx, keypair2_index);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ret = check_ethereum_keypairCtx(&keypair4_ctx, keypair2_index, "Persistkeypair2", &keypair2_config);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
    ...
    ret = check_ethereum_keypair_in_keypairList(&keypair3_ctx, &keypair_list);
    ck_assert_int_eq(ret, BOAT_SUCCESS);

    ret = check_ethereum_keypair_in_keypairList(&keypair4_ctx, &keypair_list);
    ck_assert_int_eq(ret, BOAT_SUCCESS);
