...
BOAT_RESULT ethereumGetBalance(BoatEthWallet *wallet_ptr)
{
    ...
    balance_wei = BoatEthWalletGetBalance(wallet_ptr, NULL);
    if( balance_wei == NULL ){
        BoatLog(BOAT_LOG_CRITICAL, "get balance failed.");
        return BOAT_ERROR;
    }
...
BOAT_RESULT ethereumTransfer(BoatEthWallet *wallet_ptr)
{
    ...
    result = BoatEthTransfer( &tx_ctx, "0xDE0B6B3A7640000" );
