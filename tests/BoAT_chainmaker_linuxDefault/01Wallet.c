...
START_TEST(test_001CreateWallet_0013_CreateOneTimeWalletFailureChainIdLenExceed) 
{
    BSINT32 rtnVal;
    BoatHlchainmakerWallet *g_chaninmaker_wallet_ptr = NULL;
    BoatHlchainmakerWalletConfig wallet_config = get_chainmaker_wallet_settings();
    ...
    memcpy(wallet_config.chain_id_cfg, host_name, EXCEED_STR_MAX_LEN);
    extern BoatIotSdkContext g_boat_iot_sdk_context;
...
Suite *make_wallet_suite(void) 
{
    ...
    tcase_add_test(tc_wallet_api, test_001CreateWallet_0013_CreateOneTimeWalletFailureChainIdLenExceed);
