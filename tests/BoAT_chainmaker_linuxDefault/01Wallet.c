...
START_TEST(test_001CreateWallet_0013_CreateOneTimeWalletFailureHostNameLenExceed) 
{
    BSINT32 rtnVal;
    BoatHlchainmakerWallet *g_chaninmaker_wallet_ptr = NULL;
    BoatHlchainmakerWalletConfig wallet_config = get_chainmaker_wallet_settings();

    char host_name[EXCEED_STR_MAX_LEN];
    for (int i = 0; i < EXCEED_STR_MAX_LEN; i++)
    {
        host_name[i] = 'a';
    }
    memcpy(wallet_config.host_name_cfg, host_name, EXCEED_STR_MAX_LEN);
    extern BoatIotSdkContext g_boat_iot_sdk_context;
...
START_TEST(test_001CreateWallet_0014_CreateOneTimeWalletFailureChainIdLenExceed) 
{
    ...
    char chain_id[EXCEED_STR_MAX_LEN];
    for (int i = 0; i < EXCEED_STR_MAX_LEN; i++)
    {
        chain_id[i] = 'b';
    }
    memcpy(wallet_config.chain_id_cfg, chain_id, EXCEED_STR_MAX_LEN);
    extern BoatIotSdkContext g_boat_iot_sdk_context;
...
START_TEST(test_001CreateWallet_0015_CreateOneTimeWalletFailureOrgIdLenExceed) 
{
    BSINT32 rtnVal;
    BoatHlchainmakerWallet *g_chaninmaker_wallet_ptr = NULL;
    BoatHlchainmakerWalletConfig wallet_config = get_chainmaker_wallet_settings();

    char org_id[EXCEED_STR_MAX_LEN];
    for (int i = 0; i < EXCEED_STR_MAX_LEN; i++)
    {
        org_id[i] = 'c';
    }
    memcpy(wallet_config.org_id_cfg, org_id, EXCEED_STR_MAX_LEN);
    extern BoatIotSdkContext g_boat_iot_sdk_context;
