...
#### 钱包接口
##### 钱包的数据结构和功能实现清单
钱包是一个容器，容器里除了存储着访问区块链所必须的信息，还包含了一个秘钥管理系统，它们分别对应着钱包的数据结构和实现方法。
...
+ 交易初始化：  
  该接口主要实现初始化交易结构中的字段。Ethereum的交易结构中除签名的v、r、s三个字段外，还有nonce、gasPrice、gasLimit、recipient、value、data等六个字段。请注意，交易的Nonce字段的设置并不在该接口中实现，而是放在交易发送时刻设置，因为用户可能同时创建多个交易报文，而这些报文被发送至区块链网络的顺序是不确定的。另外需要说明的是，Nonce和gasPrice字段宜通过调用协议层相应的web3接口从区块链获取，此种方式需要访问网络，会产生一定的网络流量。该接口执行的内容包括：
  1. 设置交易的GasPrice字段
  2. 设置交易的GasLimit字段
  3. 设置交易的Recipient字段
  4. 设置交易的Value字段
...
-	钱包初始化
    ...
    6)	设置节点信息，如背书节点和排序节点的地址，主机名。如果启用了TLS，主机名字段将被用来做服务端身份的验证，它和服务端TLS证书中的CN字段保持一致；如果不启用TLS，主机名将被忽略。
    7)	初始化HTTP2环境。
-	钱包反初始化
...
-	交易参数设置
    ...
    2)	设置交易的参数。
-	发送交易
    1)	判断交易参数为“query”还是“invoke”
    2)	参数为“query”则组织发送查询相关的报文
    3)	参数为“invoke”则组织发送调用相关的报文
-	查询交易回执
    1)	查询交易可通过交易参数的设置实现。
...
### 协议层
#### 概述
协议层位于BoAT SDK的第二层，主要实现各个区块链的协议部分。对于Ethereum系的区块链，其协议比较相似，如Ethereum、PlatONE和FISCOBCOS。   
...
![Fabric提案报文结构](./images/fabric-proposal.png)
![Fabric交易报文结构](./images/fabric-transaction.png)
...
![ Fabric交易流程](./images/fabric-transaction-flow.png)
#####	Fabric协议接口实现
