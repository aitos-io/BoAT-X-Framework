...
BCHAR* UtilityNative2PKCS(KeypairNative keypair){
    ...
    offset_bat = (((offset -1)/3)+1)*4;
    outStr = BoatMalloc(offset_bat + strlen(PRIKEY_PKCS_BEGIN) + strlen(PRIKEY_PKCS_END) + 6) ;
    if(NULL == outStr){
        if(dataHex != NULL){
            BoatFree(dataHex);
        }
    ...
    memset(outStr,0,offset_bat + strlen(PRIKEY_PKCS_BEGIN) + strlen(PRIKEY_PKCS_END) + 6);
