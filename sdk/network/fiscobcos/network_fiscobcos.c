 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATFiscobcos_GetNetworkFromNvram(BoatFiscobcosNetworkData *Networkdata, BUINT32 startAddr, BoatStoreType storeType, BUINT32 *outlen)
{
...
BOAT_RESULT BoATFiscobcos_GetNetworkList(BoatFiscobcosNetworkContext *networkList)
{
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_FISCOBCOS)
        { // fiscobcos
            result = BoATFiscobcos_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_FLASH, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_FISCOBCOS)
        { // fiscobcos
            result = BoATFiscobcos_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_RAM, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
...
//  * @param[inout] {BoatFiscobcosNetworkContext} *mNetworkList
//  * @return
...
//  * @author: aitos
//  */
// __BOATSTATIC BOAT_RESULT BoATFiscobcos_Init_BoatFiscobcosNetworkContext(BoatFiscobcosNetworkContext *mNetworkList){
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATFiscobcos_GetFreeNetworkIndex_From_Persistent(void)
{
 ...
 *  data like this:
 *  L  LV LV LV
 * @param[in] {BoatFiscobcosNetworkData*} networkData
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATFiscobcos_Get_NetworkData_Len(BoatFiscobcosNetworkData *networkData, BUINT32 *len)
{
 ...
 *  like this:
 *  L  LV LV LV
 * @param {BoatFiscobcosNetworkData} *mNetworkDataCtx
 * @param {BUINT8} *data
 * network data
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATFiscobcos_Get_Network_Data(BoatFiscobcosNetworkData *mNetworkDataCtx, BUINT8 *data, BUINT32 *datalen)
{
    BOAT_RESULT result = BOAT_SUCCESS;
 ...
 * @param[inout] {BoatFiscobcosNetworkData} *mNetworkDataCtx
 * @return
 *   This function returns BOAT_SUCCESS if initialization is successful.\n
 *   Otherwise it returns one of the error codes. Refer to header file boaterrcode.h
 *   for details.
 * @author: aitos
 */
BOAT_RESULT BoATFiscobcosNetworkDataInit(BoatFiscobcosNetworkData *mNetworkDataCtx)
{
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATFiscobcos_NetworkDataCtx_Store(BoatFiscobcosNetworkData *mNetworkDataCtx, BoatStoreType storeType)
{
    BOAT_RESULT result = BOAT_SUCCESS;
    BUINT8 networkNum = 0;
    BUINT32 networkLength = 0, networkLengthLen = 0, offset = 0;
    BoatProtocolType protocol = BOAT_PROTOCOL_FISCOBCOS;
    BUINT8 lengthbytes[3] = {0};
    BUINT8 networknumBytes[4] = {0};
    ...
    if (networkNum >= BOAT_MAX_NETWORK_NUM)
    {
        boat_throw(BOAT_ERROR_NETWORK_INDEX_EXCEED, fiscobcos_exception);
    }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (result != BOAT_SUCCESS)
        {
            ...
            boat_throw(result, fiscobcos_exception);
        }
    ...
    /* get network data */
    result = BoATFiscobcos_Get_NetworkData_Len(mNetworkDataCtx, &networkLength);
    if (result < BOAT_SUCCESS)
    {
        boat_throw(result, fiscobcos_exception);
    }
    ...
    if (networkData == NULL)
    {
        boat_throw(BOAT_ERROR, fiscobcos_exception);
    }
    result = BoATFiscobcos_Get_Network_Data(mNetworkDataCtx, networkData, &networkLength);
    if (result != BOAT_SUCCESS)
    {
    ...
    boat_catch(fiscobcos_exception)
    {
...
/**
 * @description:
 * This function create fiscobcos network
...
BOAT_RESULT BoATFiscobcosNetworkCreate(BoatFiscobcosNetworkConfig *networkConfig, BoatStoreType storeType)
{
    ...
    {
        result = BoATFiscobcos_GetFreeNetworkIndex_From_Persistent();
        if (result < BOAT_SUCCESS)
        {
            return result;
        }
    ...
    result = BoATFiscobcos_NetworkDataCtx_Store(&mNetworkDataCtx, storeType);
    if (result != BOAT_SUCCESS)
    {
        return result;
    }
    ...
    */
    ...
    { // persistent network
        ...
        for (i = 0; i < networkNum; i++)
        {
            ...
            if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_FISCOBCOS))
            {
                break;
            }
...
BOAT_RESULT BoATFiscobcos_GetNetworkByIndex(BoatFiscobcosNetworkData *networkData, BUINT8 index)
{
    ...
    for (BUINT8 i = 0; i < networkNum; i++)
    {
        ...
        if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_FISCOBCOS))
        {
            ...
            result = BoATFiscobcos_GetNetworkFromNvram(networkData, offset, storetype, &networkLengthLen);
            if (result != BOAT_SUCCESS)
            {
                BoatLog(BOAT_LOG_NORMAL, "read network data fail");
                return result;
