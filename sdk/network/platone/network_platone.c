 ...
 * @LastEditTime: 2022-09-27 14:57:36
 */
/******************************************************************************
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatone_GetNetworkFromNvram(BoatPlatoneNetworkData *Networkdata, BUINT32 startAddr, BoatStoreType storeType, BUINT32 *outlen)
{
...
BOAT_RESULT BoATPlatone_GetNetworkList(BoatPlatoneNetworkContext *networkList)
{
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_PLATONE)
        { // platone
            result = BoATPlatone_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_FLASH, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_PLATONE)
        { // platone
            result = BoATPlatone_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_RAM, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
...
//  * @param[inout] {BoatPlatoneNetworkContext} *mNetworkList
//  * @return
...
//  * @author: aitos
//  */
// __BOATSTATIC BOAT_RESULT BoATPlatone_Init_BoatPlatoneNetworkContext(BoatPlatoneNetworkContext *mNetworkList){
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatone_GetFreeNetworkIndex_From_Persistent(void)
{
 ...
 *  data like this:
 *  L  LV LV LV
 * @param[in] {BoatPlatoneNetworkData*} networkData
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatone_Get_NetworkData_Len(BoatPlatoneNetworkData *networkData, BUINT32 *len)
{
 ...
 *  like this:
 *  L  LV LV LV
 * @param {BoatPlatoneNetworkData} *mNetworkDataCtx
 * @param {BUINT8} *data
 * network data
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatone_Get_Network_Data(BoatPlatoneNetworkData *mNetworkDataCtx, BUINT8 *data, BUINT32 *datalen)
{
    BOAT_RESULT result = BOAT_SUCCESS;
 ...
 * @param[inout] {BoatPlatoneNetworkData} *mNetworkDataCtx
 * @return
 *   This function returns BOAT_SUCCESS if initialization is successful.\n
 *   Otherwise it returns one of the error codes. Refer to header file boaterrcode.h
 *   for details.
 * @author: aitos
 */
BOAT_RESULT BoATPlatoneNetworkDataInit(BoatPlatoneNetworkData *mNetworkDataCtx)
{
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatone_NetworkDataCtx_Store(BoatPlatoneNetworkData *mNetworkDataCtx, BoatStoreType storeType)
{
    BOAT_RESULT result = BOAT_SUCCESS;
    BUINT8 networkNum = 0;
    BUINT32 networkLength = 0, networkLengthLen = 0, offset = 0;
    BoatProtocolType protocol = BOAT_PROTOCOL_PLATONE;
    BUINT8 lengthbytes[3] = {0};
    BUINT8 networknumBytes[4] = {0};
    ...
    if (networkNum >= BOAT_MAX_NETWORK_NUM)
    {
        boat_throw(BOAT_ERROR_NETWORK_INDEX_EXCEED, platone_exception);
    }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (result != BOAT_SUCCESS)
        {
            ...
            boat_throw(result, platone_exception);
        }
    ...
    /* get network data */
    result = BoATPlatone_Get_NetworkData_Len(mNetworkDataCtx, &networkLength);
    if (result < BOAT_SUCCESS)
    {
        boat_throw(result, platone_exception);
    }
    ...
    if (networkData == NULL)
    {
        boat_throw(BOAT_ERROR, platone_exception);
    }
    result = BoATPlatone_Get_Network_Data(mNetworkDataCtx, networkData, &networkLength);
    if (result != BOAT_SUCCESS)
    {
...
BOAT_RESULT BoATPlatoneNetworkCreate(BoatPlatoneNetworkConfig *networkConfig, BoatStoreType storeType)
{
    ...
    result = BoATPlatoneNetworkDataInit(&mNetworkDataCtx);
    if (result != BOAT_SUCCESS)
    {
        return result;
    }
    ...
    {
        result = BoATPlatone_GetFreeNetworkIndex_From_Persistent();
        if (result < BOAT_SUCCESS)
        {
            return result;
        }
    ...
    result = BoATPlatone_NetworkDataCtx_Store(&mNetworkDataCtx, storeType);
    if (result != BOAT_SUCCESS)
    {
        return result;
    }
    ...
    */
    ...
    { // persistent network
        ...
        for (i = 0; i < networkNum; i++)
        {
            ...
            if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_PLATONE))
            {
                break;
            }
...
BOAT_RESULT BoATPlatone_GetNetworkByIndex(BoatPlatoneNetworkData *networkData, BUINT8 index)
{
    ...
    for (BUINT8 i = 0; i < networkNum; i++)
    {
        ...
        if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_PLATONE))
        {
            ...
            result = BoATPlatone_GetNetworkFromNvram(networkData, offset, storetype, &networkLengthLen);
            if (result != BOAT_SUCCESS)
            {
                BoatLog(BOAT_LOG_NORMAL, "read network data fail");
                return result;
