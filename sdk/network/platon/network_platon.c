 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatON_GetNetworkFromNvram(BoatPlatONNetworkData *Networkdata, BUINT32 startAddr, BoatStoreType storeType, BUINT32 *outlen)
{
...
BOAT_RESULT BoATPlatON_GetNetworkList(BoatPlatONNetworkContext *networkList)
{
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_PLATON)
        { // platon
            result = BoATPlatON_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_FLASH, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (protocolType == BOAT_PROTOCOL_PLATON)
        { // platon
            result = BoATPlatON_GetNetworkFromNvram(&(networkList->networks[index++]), offset, BOAT_STORE_TYPE_RAM, &networkDataLength);
            if (result != BOAT_SUCCESS)
            {
                return result;
            }
...
//  * @param[inout] {BoatPlatONNetworkContext} *mNetworkList
//  * @return
...
//  * @author: aitos
//  */
// __BOATSTATIC BOAT_RESULT BoATPlatON_Init_BoatPlatONNetworkContext(BoatPlatONNetworkContext *mNetworkList){
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatON_GetFreeNetworkIndex_From_Persistent(void)
{
 ...
 *  data like this:
 *  L  LV LV LV
 * @param[in] {BoatPlatONNetworkData*} networkData
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatON_Get_NetworkData_Len(BoatPlatONNetworkData *networkData, BUINT32 *len)
{
 ...
 *  like this:
 *  L  LV LV LV
 * @param {BoatPlatONNetworkData} *mNetworkDataCtx
 * @param {BUINT8} *data
 * network data
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatON_Get_Network_Data(BoatPlatONNetworkData *mNetworkDataCtx, BUINT8 *data, BUINT32 *datalen)
{
    BOAT_RESULT result = BOAT_SUCCESS;
 ...
 * @param[inout] {BoatPlatONNetworkData} *mNetworkDataCtx
 * @return
 *   This function returns BOAT_SUCCESS if initialization is successful.\n
 *   Otherwise it returns one of the error codes. Refer to header file boaterrcode.h
 *   for details.
 * @author: aitos
 */
BOAT_RESULT BoATPlatONNetworkDataInit(BoatPlatONNetworkData *mNetworkDataCtx)
{
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATPlatON_NetworkDataCtx_Store(BoatPlatONNetworkData *mNetworkDataCtx, BoatStoreType storeType)
{
    BOAT_RESULT result = BOAT_SUCCESS;
    BUINT8 networkNum = 0;
    BUINT32 networkLength = 0, networkLengthLen = 0, offset = 0;
    BoatProtocolType protocol = BOAT_PROTOCOL_PLATON;
    BUINT8 lengthbytes[3] = {0};
    BUINT8 networknumBytes[4] = {0};
    ...
    if (networkNum >= BOAT_MAX_NETWORK_NUM)
    {
        boat_throw(BOAT_ERROR_NETWORK_INDEX_EXCEED, platon_exception);
    }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if (result != BOAT_SUCCESS)
        {
            ...
            boat_throw(result, platon_exception);
        }
    ...
    /* get network data */
    result = BoATPlatON_Get_NetworkData_Len(mNetworkDataCtx, &networkLength);
    if (result < BOAT_SUCCESS)
    {
        boat_throw(result, platon_exception);
    }
    ...
    if (networkData == NULL)
    {
        boat_throw(BOAT_ERROR, platon_exception);
    }
    result = BoATPlatON_Get_Network_Data(mNetworkDataCtx, networkData, &networkLength);
    if (result != BOAT_SUCCESS)
    {
...
BOAT_RESULT BoATPlatONNetworkCreate(BoatPlatONNetworkConfig *networkConfig, BoatStoreType storeType)
{
    ...
    {
        result = BoATPlatON_GetFreeNetworkIndex_From_Persistent();
        if (result < BOAT_SUCCESS)
        {
            return result;
        }
    ...
    result = BoATPlatON_NetworkDataCtx_Store(&mNetworkDataCtx, storeType);
    if (result != BOAT_SUCCESS)
    {
        return result;
    }
    ...
    */
    ...
    { // persistent network
        ...
        for (i = 0; i < networkNum; i++)
        {
            ...
            if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_PLATON))
            {
                break;
            }
...
BOAT_RESULT BoATPlatON_GetNetworkByIndex(BoatPlatONNetworkData *networkData, BUINT8 index)
{
    ...
    for (BUINT8 i = 0; i < networkNum; i++)
    {
        ...
        if ((networkIndex == index) && (protocolType == BOAT_PROTOCOL_PLATON))
        {
            ...
            result = BoATPlatON_GetNetworkFromNvram(networkData, offset, storetype, &networkLengthLen);
            if (result != BOAT_SUCCESS)
            {
                BoatLog(BOAT_LOG_NORMAL, "read network data fail");
                return result;
