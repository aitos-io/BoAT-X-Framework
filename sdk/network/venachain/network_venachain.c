 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATVenachain_GetNetworkFromNvram(BoatVenachainNetworkData *Networkdata , BUINT32 startAddr,BoatStoreType storeType,BUINT32 *outlen)
{
...
BOAT_RESULT BoATVenachain_GetNetworkList(BoatVenachainNetworkContext *networkList)
{
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        if(protocolType == BOAT_PROTOCOL_VENACHAIN){  // venachain
            result = BoATVenachain_GetNetworkFromNvram(&(networkList->networks[index++]),offset,BOAT_STORE_TYPE_FLASH,&networkDataLength);
            if(result != BOAT_SUCCESS){
                return result;
            }
    ...
    for (int i = 0; i < networkNum; i++)
    {
        ...
        offset += sizeof(BUINT8);         // index
        if(protocolType == BOAT_PROTOCOL_VENACHAIN){  // venachain
            result = BoATVenachain_GetNetworkFromNvram(&(networkList->networks[index++]),offset,BOAT_STORE_TYPE_RAM,&networkDataLength);
            if(result != BOAT_SUCCESS){
                return result;
            }
            offset += networkDataLength;
...
//  * @param[inout] {BoatVenachainNetworkContext} *mNetworkList
//  * @return 
...
//  * @author: aitos
//  */
// __BOATSTATIC BOAT_RESULT BoATVenachain_Init_BoatVenachainNetworkContext(BoatVenachainNetworkContext *mNetworkList){
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATVenachain_GetFreeNetworkIndex_From_Persistent(void)
{
 ...
 *  data like this:
 *  L  LV LV LV 
 * @param[in] {BoatVenachainNetworkData*} networkData
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATVenachain_Get_NetworkData_Len(BoatVenachainNetworkData *networkData,BUINT32 *len)
{
 ...
 *  like this:
 *  L  LV LV LV 
 * @param {BoatVenachainNetworkData} *mNetworkDataCtx
 * @param {BUINT8} *data
 * network data
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATVenachain_Get_Network_Data(BoatVenachainNetworkData *mNetworkDataCtx,BUINT8 *data,BUINT32 *datalen)
{
    BOAT_RESULT result = BOAT_SUCCESS;
 ...
 * @param[inout] {BoatVenachainNetworkData} *mNetworkDataCtx
 * @return 
 *   This function returns BOAT_SUCCESS if initialization is successful.\n
 *   Otherwise it returns one of the error codes. Refer to header file boaterrcode.h 
 *   for details.
 * @author: aitos
 */
BOAT_RESULT BoATVenachainNetworkDataInit(BoatVenachainNetworkData *mNetworkDataCtx){
 ...
 * @author: aitos
 */
__BOATSTATIC BOAT_RESULT BoATVenachain_NetworkDataCtx_Store(BoatVenachainNetworkData *mNetworkDataCtx,BoatStoreType storeType)
{
    BOAT_RESULT result = BOAT_SUCCESS;
    BUINT8 networkNum = 0;
    BUINT32 networkLength = 0 ,networkLengthLen = 0, offset = 0 ;
    BoatProtocolType protocol = BOAT_PROTOCOL_VENACHAIN;
    BUINT8 lengthbytes[3] = {0};
    BUINT8 networknumBytes[4] = {0};
	...
	boat_catch(venachain_exception)
	{
...
/**
 * @description: 
 * This function creat venachain network
...
BOAT_RESULT BoatVenachainNetworkCreate(BoatVenachainNetworkConfig *networkConfig,BoatStoreType storeType)
{
    ...
    result = BoATVenachainNetworkDataInit(&mNetworkDataCtx);
    if(result != BOAT_SUCCESS){
        return result;
    }
    if(storeType == BOAT_STORE_TYPE_FLASH){
        result = BoATVenachain_GetFreeNetworkIndex_From_Persistent();
        if(result < BOAT_SUCCESS){
            return result;
        }
    ...
    result = BoATVenachain_NetworkDataCtx_Store(&mNetworkDataCtx,storeType);
    if(result != BOAT_SUCCESS){
        return result;
    }
    return networkIndex;
...
BOAT_RESULT BoATVenachainNetworkDelete(BUINT8 index)
{
    ...
    */
    ...
    for ( i = 0; i < networkNum; i++)
    {
        ...
        if((networkIndex == index) && (protocolType == BOAT_PROTOCOL_VENACHAIN)){
            break;
        }
...
BOAT_RESULT BoATVenachain_GetNetworkByIndex(BoatVenachainNetworkData *networkData ,BUINT8 index)
{
	...
	for (BUINT8 i = 0; i < networkNum; i++)
	{
        ...
        if((networkIndex == index)&&(protocolType == BOAT_PROTOCOL_VENACHAIN)){
            ...
            result = BoATVenachain_GetNetworkFromNvram(networkData,offset,storetype,&networkLengthLen);
            if(result != BOAT_SUCCESS){
