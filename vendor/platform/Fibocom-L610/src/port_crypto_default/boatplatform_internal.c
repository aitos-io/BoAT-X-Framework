...
BOAT_RESULT BoatWriteStorage(BUINT32 offset, BUINT8 *writeBuf, BUINT32 writeLen, void *rsvd)
{
    ...
    file_fd = fibo_file_open(BOAT_FILE_STOREDATA, FS_O_RDWR | FS_O_CREAT);
    if (file_fd < 0)
    {
        BoatLog(BOAT_LOG_CRITICAL, "Failed to open file: %s.", BOAT_FILE_STOREDATA);
        return BOAT_ERROR_STORAGE_FILE_OPEN_FAIL;
    ...
    if (size < offset)
	{
        ...
        fibo_file_seek(file_fd, offset, FS_SEEK_SET);
        count = fibo_file_write(file_fd, (const void *)buf_zero, offset - size);
        BoatFree(buf_zero);
        ...
        fibo_file_seek(file_fd, 0, FS_SEEK_END);
	}
	else
	{
		fibo_file_seek(file_fd, offset, FS_SEEK_SET);
