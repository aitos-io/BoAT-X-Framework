...
#define BoatLog(level, format,...)\
    do{\
        if( level <= BOAT_LOG_LEVEL ) {opencpu_printf("%s: "__FILE__":%d, %s(): "format"\n", g_log_level_name_str[level-1], __LINE__, __func__, ##__VA_ARGS__);}\
    }while(0)
#endif
...
#define BoatLog_hexdump(level, title, buf, len)\
	do{\
		if( level <= BOAT_LOG_LEVEL ){\
			opencpu_printf( "%s: %s[%03d]: ", g_log_level_name_str[level-1], title, len );\
			if( len > 0 ){\
				for( int i = 0; i < len; i++ ){opencpu_printf( "%c%c", "0123456789ABCDEF" [buf[i] / 16],"0123456789ABCDEF" [buf[i] % 16] );}\
			}else{\
				opencpu_printf("(nil)");\
			}\
			opencpu_printf( "\n" );\
