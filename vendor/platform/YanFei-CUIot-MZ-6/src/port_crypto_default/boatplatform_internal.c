						   ...
						   BUINT8 *writeBuf, BUINT32 writeLen, void *rsvd)
{
	...
	file_fd = vfs_open(fileName, O_WRONLY | O_CREAT);
	BoatLog(BOAT_LOG_NORMAL, "===003 file_fd=%d", file_fd);
	if (file_fd >= 0)
	{
		count = vfs_write(file_fd, (const void *)writeBuf, writeLen);
		BoatLog(BOAT_LOG_NORMAL, "===writeLen=%d", writeLen);
		BoatLog(BOAT_LOG_NORMAL, "===count=%d", count);
		vfs_close(file_fd);
	}
	else
	{
						  ...
						  BUINT8 *readBuf, BUINT32 readLen, void *rsvd)
{
	...
	file_fd = vfs_open(fileName, O_RDONLY);
	BoatLog(BOAT_LOG_CRITICAL, "===103 file_fd=%d", file_fd);
	if (file_fd >= 0)
	{
		count = vfs_read(file_fd, readBuf, readLen);
		vfs_close(file_fd);
	}
	else
	{
