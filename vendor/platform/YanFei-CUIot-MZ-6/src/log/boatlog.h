...
#define BoatLog(level, format,...)\
    do{\
        if( level <= BOAT_LOG_LEVEL ) {MG_osiTracePrintf(LOG_TAG, "%s: "__FILE__":%d, %s(): "format"\n", g_log_level_name_str[level-1], __LINE__, __func__, ##__VA_ARGS__);}\
    }while(0)
#endif
...
#define BoatLog_hexdump(level, title, buf, len)\
	do{\
		if( level <= BOAT_LOG_LEVEL ){\
			onemo_sys_log( "%s: %s[%03d]: ", g_log_level_name_str[level-1], title, len );\
			if( len > 0 ){\
				for( int i = 0; i < len; i++ ){onemo_sys_log( "%c%c", "0123456789ABCDEF" [buf[i] / 16],"0123456789ABCDEF" [buf[i] % 16] );}\
			}else{\
				onemo_sys_log("(nil)");\
			}\
			onemo_sys_log( "\n" );\
