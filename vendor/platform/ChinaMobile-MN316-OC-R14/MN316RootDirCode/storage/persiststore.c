							 ...
							 BUINT32 data_len)
{
    ...
    if (result == BOAT_SUCCESS)
    {
		...
		BoatLog(BOAT_LOG_VERBOSE, "ready to call BoatWriteFile, writeDataLen = %d.",writeDataLen);
							...
							BUINT32 len_to_read)
{
    ...
    BOAT_RESULT result = BOAT_SUCCESS;
	BoatLog(BOAT_LOG_VERBOSE, "at beginning len_to_read = %d\n.", len_to_read);
	...
	fileSize = BOAT_STORAGE_SALT_SIZE + sizeof(original_data_hash_array) + data_with_padding_len;
	...
	BoatLog(BOAT_LOG_VERBOSE, "BoatAesDecrypt result= %d\n.", result);
