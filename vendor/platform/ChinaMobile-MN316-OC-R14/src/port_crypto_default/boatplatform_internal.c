						  ...
						  BUINT8 *writeBuf, BUINT32 writeLen, void *rsvd)
{
	...
	if (count != writeLen)
	{
		BoatLog(BOAT_LOG_CRITICAL, "Failed to write flash");
						 ...
						 BUINT8 *readBuf, BUINT32 readLen, void *rsvd)
{
	...
	count = cm_flash_read(0, readBuf, readLen);
	if (count != readLen)
	{
		BoatLog(BOAT_LOG_CRITICAL, "Failed to read flash");
...
{
	BUINT8 writeBuf[48 + sizeof(BoatEthWalletConfig)];
	BUINT32 writeLen = 48 + sizeof(BoatEthWalletConfig);
	(void)rsvd;
