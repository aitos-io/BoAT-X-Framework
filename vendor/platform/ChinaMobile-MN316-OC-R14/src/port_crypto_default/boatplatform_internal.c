...
BOAT_RESULT BoatGetStorageSize(BUINT32 *size, void *rsvd)
{
	...
	if (cm_flash_info(&flash_info) != 0)
	{
		BoatLog(BOAT_LOG_CRITICAL, "get flash info fail.");
		return BOAT_ERROR;
	}
	*size = flash_info.block_amount * flash_info.block_size;
...
BOAT_RESULT BoatWriteStorage(BUINT32 offset, BUINT8 *writeBuf, BUINT32 writeLen, void *rsvd)
{
	...
	if (offset + writeLen > FLASH_MAX_SIZE)
	{
		BoatLog(BOAT_LOG_CRITICAL, "data to write out of memory");
		return BOAT_ERROR_COMMON_OUT_OF_MEMORY;
	}
	count = cm_flash_write(offset, writeBuf, writeLen);
	if (count != writeLen)
	{
		BoatLog(BOAT_LOG_CRITICAL, "write data to flash fail");
		return BOAT_ERROR;
	}
...
BOAT_RESULT BoatReadStorage(BUINT32 offset, BUINT8 *readBuf, BUINT32 readLen, void *rsvd)
{
	...
	if (offset + readLen > FLASH_MAX_SIZE)
	{
		BoatLog(BOAT_LOG_CRITICAL, "data to read out of memory");
		return BOAT_ERROR_COMMON_OUT_OF_MEMORY;
	}
	count = cm_flash_read(offset, readBuf, readLen);
	if (count != readLen)
	{
