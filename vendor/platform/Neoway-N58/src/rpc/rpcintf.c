...
void * RpcInit(void)
{
    ...
    rpc_context_ptr = NwyHttpPortInit();
#endif
...
#if RPC_USE_NWYHTTPPORT == 1
    NwyHttpPortDeinit(rpc_context_ptr);
#endif
...
#if RPC_USE_NWYHTTPPORT == 1
    result = NwyHttpPortSetOpt((NwyHttpPortContext *)rpc_context_ptr, remote_url_str);
#endif
...
#if RPC_USE_NWYHTTPPORT == 1
    result = NwyHttpPortRequestSync(rpc_context_ptr, (const BCHAR *)request_ptr, request_len, (BOAT_OUT BCHAR **)response_pptr, response_len_ptr);
#endif
