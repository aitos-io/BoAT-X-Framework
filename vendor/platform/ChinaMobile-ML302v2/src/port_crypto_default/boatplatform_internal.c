...
BOAT_RESULT BoatGetFileSize(const BCHAR *fileName, BUINT32 *size, void *rsvd)
{
	...
	result = cm_fs_stat(fileName, &st);
	if (result != 0)
	{
		BoatLog(BOAT_LOG_CRITICAL, "Failed to get file status: %s.", fileName);
						  ...
						  BUINT8 *writeBuf, BUINT32 writeLen, void *rsvd)
{
	...
	file = cm_fs_fopen(fileName, O_WRONLY | O_CREAT);
	if (file < 0)
	{
						 ...
						 BUINT8 *readBuf, BUINT32 readLen, void *rsvd)
{
	...
	file = cm_fs_fopen(fileName, O_RDONLY);
	if (file < 0)
	{
