...
#define BoatLog_hexdump(level, title, buf, len)\
	do{\
		if( level <= BOAT_LOG_LEVEL ){\
			cm_sys_log( "%s: %s[%03d]: ", g_log_level_name_str[level-1], title, len );\
			if( len > 0 ){\
				for( int i = 0; i < len; i++ ){cm_sys_log( "%c%c", "0123456789ABCDEF" [buf[i] / 16],"0123456789ABCDEF" [buf[i] % 16] );}\
			}else{\
				cm_sys_log("(nil)");\
			}\
			cm_sys_log( "\n" );\
