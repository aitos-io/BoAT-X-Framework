...
BOAT_RESULT random_stream(BUINT8* buf, BUINT16 len)
{
	...
	if (0 == wc_RNG_GenerateBlock(rng, buf, len))
	{
		ret = BOAT_SUCCESS;
	}
...
BOAT_RESULT  BoatGetFileSize(const BCHAR* fileName, BUINT32* size, void* rsvd)
{
	size_t fs;
	int fid;
	BUINT16 prikey_len = 0;

	if ((fileName == NULL) || (size == NULL))
	{
		BoatLog(BOAT_LOG_NORMAL, "param which 'fileName' or 'size' can't be NULL.");
		return BOAT_ERROR_COMMON_INVALID_ARGUMENT;
	}

	fid = Storage_OpenMutableFile();

	if (fid < 0)
	{
		BoatLog(BOAT_LOG_NORMAL, "Failed to open file: %s.", fileName);
	...
	if (fs > 0 && NULL == rsvd)
	{
		prikey_len = BoatCountPrikeyLen(fid);
	}
	...
	BUINT8* writeBuf, BUINT32 writeLen, void* rsvd)
{
	...
	{
		BoatLog(BOAT_LOG_NORMAL, "param which 'fileName' or 'writeBuf' can't be NULL.");
		return BOAT_ERROR_COMMON_INVALID_ARGUMENT;
	}
	...
	if (fid < 0)
	{
		BoatLog(BOAT_LOG_NORMAL, "Error: Openning %s failed!\n", fileName);
		return BOAT_ERROR_STORAGE_FILE_OPEN_FAIL;
	}
	...
	if (NULL == rsvd)
	{
		count = BoatCountPrikeyLen(fid);
		lseek(fid, count, SEEK_SET);
	}
	*/
	count = write(fid, writeBuf, writeLen);

	if (count < 0)
	{
		result = BOAT_ERROR_STORAGE_FILE_WRITE_FAIL;
		...
		BoatLog(BOAT_LOG_NORMAL, "ERROR: An error occurred while writing to mutable file");
	}
	else if (count < writeLen)
	{
		result = BOAT_ERROR_STORAGE_FILE_WRITE_FAIL;
		...
		// written.
		BoatLog(BOAT_LOG_VERBOSE, "ERROR: Only wrote %d of %d bytes requested\n", count, writeLen);
	}
	else
	{
		result = BOAT_SUCCESS;
		BoatLog(BOAT_LOG_VERBOSE, "INFO: WriteStorage: writeLen=%d, %s.\r\n", writeLen, writeBuf);
	}
	close(fid);
	...
	BUINT8* readBuf, BUINT32 readLen, void* rsvd)
{
	...
	{
		BoatLog(BOAT_LOG_NORMAL, "param which 'fileName' or 'readBuf' can't be NULL.");
		return BOAT_ERROR_COMMON_INVALID_ARGUMENT;
	}
	...
	/*
	if (NULL != rsvd)// read prikey
	{
		readLen = BoatCountPrikeyLen(fid);
	...
	*/
	count = read(fid, readBuf + 1, readLen);
	close(fid);

	if (NULL != rsvd && (NULL == strstr(readBuf, "-----END EC PRIVATE KEY-----")))// read prikey
	{
		result = BOAT_ERROR;
	}
...
BOAT_RESULT  BoatRemoveFile(const BCHAR* fileName, void* rsvd)
{
	...
	if (ret < 0)
	{
		BoatLog(BOAT_LOG_NORMAL, "An error occurred while deleting the mutable file");
	}
	else
	{
		BoatLog(BOAT_LOG_NORMAL, "Successfully deleted the mutable file!\n");
	}
