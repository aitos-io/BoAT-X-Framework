							  ...
							  BUINT32 data_len )
{
	...
	// aes init
	result = aes_encrypt_key256( g_aes_key, &ctxe );
    ...
    // Encrypt the data		
	result = aes_cbc_encrypt(rawData, encrypted_array, encrypted_len, salt_array, &ctxe);
							 ...
							 BUINT32 len_to_read )
{
	...
	//aes init
	result  = aes_decrypt_key256( g_aes_key, &ctxd );
	 	...
	 	( fileSize >= BOAT_STORAGE_SALT_SIZE + sizeof(data_hash_array) + len_to_read ) )
	{
		...
		result = aes_cbc_decrypt(encrypted_array, plain_array, encrypted_readLen, salt_array, &ctxd);
		BoatLog_hexasciidump(BOAT_LOG_CRITICAL,"test", plain_array, encrypted_readLen);
